...
def _mean_welch(values: np.ndarray, sampling_rate_hz: float) -> tuple[np.ndarray, np.ndarray]:
...
def _observed_peaks(frequencies: np.ndarray, power: np.ndarray) -> list[float]:
    ...
    local_peaks, _ = signal.find_peaks(
        masked_power,
        distance=3,
        prominence=max(float(np.median(masked_power) * 5), np.finfo(float).eps),
    )
...
def run_experiment(recipe: ExperimentRecipe) -> ExperimentResult:
    ...
    raw_frequency, raw_power = _mean_welch(simulation.sensor_uv, recipe.simulation.sampling_rate_hz)
    filtered_frequency, filtered_power = _mean_welch(
    ...
    traces = {
        "Raw · EEG 01": np.round(simulation.sensor_uv[0, view], 4).tolist(),
        "Filtered · EEG 01": np.round(filtered.values_uv[0, view], 4).tolist(),
    }
