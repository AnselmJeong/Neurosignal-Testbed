...
import { RealDataTracePanel } from './RealDataTracePanel'

const STEPS = [
  { label: 'Resolve', description: 'Locate a supported recording safely.', icon: FolderInput },
  { label: 'Inspect', description: 'Review metadata before creating files.', icon: FileSearch },
  { label: 'Copy', description: 'Create a separate FIF working copy.', icon: HardDrive },
  { label: 'QC', description: 'Compute descriptive quality checks.', icon: ClipboardCheck },
  { label: 'Report', description: 'Export provenance and QC evidence.', icon: FileOutput },
  { label: 'Review', description: 'Interpret findings without clinical claims.', icon: ShieldCheck },
] as const
...
export function RealDataWorkbench({ activeLab, onLabChange }: { activeLab: LabId; onLabChange: (lab: LabId) => void }) {
  ...
  useEffect(() => {
    if (result) window.localStorage.setItem(ACTIVE_REAL_PROJECT_KEY, result.project.project_id)
  }, [result])
  ...
  return <>
    <main className="workspace real-data-workspace">
      ...
      <section className="canvas-panel real-data-canvas"><div className="canvas-toolbar real-data-toolbar"><div><span className="eyebrow">Immutable source → FIF derivative → QC record</span><h2>{result ? `${result.project.project_name} · ready for report` : inspection ? `${inspection.source_name} · inspected read-only` : 'Select a local recording'}</h2></div><span className="real-data-status">{result ? 'WORKING COPY READY' : inspection ? 'INSPECTED' : 'AWAITING FILE'}</span></div><div className="real-data-stage">{error && <div className="empty-stage error-stage"><AlertTriangle size={30} /><h3>Import stopped safely</h3><p>{error}</p></div>}{!error && !inspection && !result && <div className="real-data-empty"><HardDrive size={42} /><span className="eyebrow">Original data stays untouched</span><h3>Resolve the file before any analysis.</h3><p>Metadata, channels, annotations, unit assumptions, and sensor locations are surfaced before NeuroSignal writes its FIF derivative.</p></div>}{!error && (inspection || result) && (result ? <div className="real-data-review-content"><details className="real-data-import-details"><summary>Import evidence · {result.inspection.channel_count} channels · {result.inspection.duration_s.toFixed(1)} s</summary><InspectionCanvas inspection={result.inspection} result={result} /></details><RealDataTracePanel projectId={result.project.project_id} /></div> : <InspectionCanvas inspection={inspection!} result={null} />)}</div><div className="canvas-readout real-data-readout"><div><small>EEG CHANNELS</small><strong>{(result?.inspection ?? inspection)?.eeg_channel_count ?? '—'}</strong></div><div><small>SAMPLING RATE</small><strong>{(result?.inspection ?? inspection) ? `${(result?.inspection ?? inspection)?.sampling_rate_hz.toFixed(0)} Hz` : '—'}</strong></div><div><small>ANNOTATIONS</small><strong>{(result?.inspection ?? inspection)?.annotation_count ?? '—'}</strong></div><div><small>WORKING COPY</small><strong>{result ? 'FIF' : '—'}</strong></div></div></section>
      <aside className="inspector-panel real-data-inspector"><section className="inspector-lead"><span className="eyebrow">Interpretation</span><h2>{result ? 'QC informs review; it does not decide.' : 'Metadata is evidence.'}</h2><p>{result ? 'The report records observed file properties, QC summaries, provenance, and caveats. It makes no recovery claim.' : 'Inspect the import evidence before deciding whether a montage, annotation, or preprocessing step is justified.'}</p></section>{warnings.map((warning) => <section className={`warning-card ${warning.severity}`} key={warning.code}><div><AlertTriangle size={16} /><strong>{warning.title}</strong></div><p>{warning.explanation}</p>{warning.suggestion && <small>{warning.suggestion}</small>}</section>)}{result && <section className="qc-summary"><div className="section-title"><span>QC summary</span><small>DESCRIPTIVE</small></div><dl><div><dt>Potentially flat</dt><dd>{result.qc.flat_channels.join(', ') || 'None'}</dd></div><div><dt>Marked bad</dt><dd>{result.qc.bad_channels.join(', ') || 'None'}</dd></div><div><dt>8–12 Hz relative power</dt><dd>{result.qc.alpha_relative_power?.toFixed(3) ?? '—'}</dd></div><div><dt>Schema</dt><dd>v{result.project.schema_version} · {result.project.migration_status}</dd></div></dl><button className="primary-button fit-button" onClick={exportReport} disabled={busy !== null}><Download size={15} /> {busy === 'report' ? 'Exporting' : 'Export HTML report'}</button><button className="quiet-button fit-button" onClick={() => onLabChange('qeeg')}>Continue to QEEG analysis</button></section>}<div className="nonclinical"><AlertTriangle size={15} /><span>Local descriptive QC only.<br />Not clinical diagnosis.</span></div></aside>
    </main>
