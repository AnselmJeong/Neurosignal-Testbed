...
export function RealDataWorkbench({ activeLab, onLabChange }: { activeLab: LabId; onLabChange: (lab: LabId) => void }) {
  ...
  return <>
    <main className="workspace real-data-workspace">
      ...
      <section className="canvas-panel real-data-canvas"><div className="canvas-toolbar real-data-toolbar"><div><span className="eyebrow">Immutable source → FIF derivative → QC record</span><h2>{result ? `${result.project.project_name} · ready for report` : inspection ? `${inspection.source_name} · inspected read-only` : 'Select a local recording'}</h2></div><span className="real-data-status">{result ? 'WORKING COPY READY' : inspection ? 'INSPECTED' : 'AWAITING FILE'}</span></div><div className="real-data-stage">{error && <div className="empty-stage error-stage"><AlertTriangle size={30} /><h3>Import stopped safely</h3><p>{error}</p></div>}{!error && !inspection && !result && <div className="real-data-empty"><HardDrive size={42} /><span className="eyebrow">Original data stays untouched</span><h3>Resolve the file before any analysis.</h3><p>Metadata, channels, annotations, unit assumptions, and sensor locations are surfaced before NeuroSignal writes its FIF derivative.</p></div>}{!error && (inspection || result) && <InspectionCanvas inspection={result?.inspection ?? inspection!} result={result} />}</div><div className="canvas-readout real-data-readout"><div><small>EEG CHANNELS</small><strong>{(result?.inspection ?? inspection)?.eeg_channel_count ?? '—'}</strong></div><div><small>SAMPLING RATE</small><strong>{(result?.inspection ?? inspection) ? `${(result?.inspection ?? inspection)?.sampling_rate_hz.toFixed(0)} Hz` : '—'}</strong></div><div><small>ANNOTATIONS</small><strong>{(result?.inspection ?? inspection)?.annotation_count ?? '—'}</strong></div><div><small>WORKING COPY</small><strong>{result ? 'FIF' : '—'}</strong></div></div></section>
